...
#'   words <- apply(x_emb, 1, function(x) paste(rev(rank(x, 
#' ties.method=tie_method)), collapse="-"))
#'   table(words)/nrow(x_emb)
#' }
word_distribution <- function(x_emb, tie_method) {
  words <-  unlist(lapply(lapply(1:nrow(x_emb), function(x) 
    (rev(rank(-x_emb[x,])))), paste, collapse="-"))
  table(words)/nrow(x_emb)
}
...
weighted_word_distribution <- function(x_emb, tie_method) {
  words <- apply(x_emb, 1, function(x) paste(rev(
    rank(x, ties.method = tie_method)), collapse="-"))
...
rolling_PE <- function(x, weighted, word_length, tau, tie_method, 
  ...
  words <- apply(x_emb, 1, function(x) paste(
    rev(rank(x, ties.method = tie_method, na.last="keep")), collapse="-"))
  weights <- apply(x_emb, 1, function(x) var(x))
  ...
  if(weighted)
    PE <- rollapply(data.frame(weights, words),
                    width=windowsize,
                    function(z) entropy(aggregate(as.numeric(z[,1]), 
                                                  list(z[,2]), sum)$x / 
